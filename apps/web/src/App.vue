...
import { Bell, Menu, Moon, RotateCw, Sun, X } from '@lucide/vue'
...
const dueReminders = computed(() =>
  store.tasks.filter((task) => {
    ...
    return (
      new Date(task.remind_at).getTime() <= now.value && !dismissedReminders.value.includes(key)
    )
  }),
)
...
function reminderKey(taskId: string, remindAt: string) {
  return `${taskId}:${remindAt}`
}

function loadDismissedReminders(): string[] {
  try {
    const stored = JSON.parse(localStorage.getItem('prosepect.dismissed-reminders') ?? '[]')
...
<template>
  <div class="flex min-h-dvh bg-white text-slate-950 dark:bg-slate-950 dark:text-slate-50">
    <div
      v-if="sidebarOpen"
